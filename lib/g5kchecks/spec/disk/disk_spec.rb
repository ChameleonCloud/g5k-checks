describe "Disk" do
  ...
  RSpec.configuration.node.ohai_description["block_device"].select { |key,value| key =~ /[sh]d.*/ and value["model"] != "vmDisk" }.each { |k,v|
    ...
    it "should have the correct name" do
      ...
      expect(name_api).to eql(nil), "#{k}, not_exist, block_devices, #{k}, device"
    end
    ...
    it "should have the correct size" do
      ...
      expect(size_ohai).to eql(size_api), "#{size_ohai}, #{size_api}, block_devices, #{k}, size"
    end
    ...
    it "should have the correct model" do
      ...
      expect(model_ohai).to eql(model_api), "#{model_ohai}, #{model_api}, block_devices, #{k}, model"
    end
    ...
    it "should have the correct revision" do
      ...
      expect(version_ohai).to eql(version_api), "#{version_ohai}, #{version_api}, block_devices, #{k}, rev"
    end
